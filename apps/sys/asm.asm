...
msg_mem_prompt: db "Select App Memory Size:",13,10
        db "1. Tiny   (4KB)",13,10
        db "2. Small  (8KB)",13,10
        db "3. Normal (16KB)",13,10
        db "4. High   (32KB)",13,10
        db "5. Big    (48KB)",13,10
        db "6. Huge   (64KB)",13,10
        db "> ",0
app_sp: dw 0xFFE0       ; 默认 SP 设为 64KB
...
___assumelabel:
    call getsymbol
    cmp cx,0
    jne ___assumelabel_found
    mov ax,0
    mov cx,1
___assumelabel_found:
    pop cx
    xchg ax,cx
    push cx
___test2address:
...
.wait_key:
    ...
    cmp al, '1'
    jne .chk2
    mov word [app_sp], 0x0FE0       ; 4KB
    jmp ___nextpass
.chk2:
    cmp al, '2'
    jne .chk3
    mov word [app_sp], 0x1FE0       ; 8KB
    jmp ___nextpass
.chk3:
    cmp al, '3'
    jne .chk4
    mov word [app_sp], 0x3FE0       ; 16KB
    jmp ___nextpass
.chk4:
    cmp al, '4'
    jne .chk5
    mov word [app_sp], 0x7FE0       ; 32KB
    jmp ___nextpass
.chk5:
    cmp al, '5'
    jne .chk6
    mov word [app_sp], 0xBFE0       ; 48KB
    jmp ___nextpass
.chk6:
    mov word [app_sp], 0xFFE0       ; 64KB
