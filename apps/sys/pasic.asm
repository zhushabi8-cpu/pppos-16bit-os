...
.exec_buffer:
    ...
    cmp al, 1
    je .buf_on
    mov word [cs:draw_seg], 0xA000
    jmp .next_exec
.buf_on:
    push ax
    mov ah, 0x33
    int 0x80
    mov word [cs:draw_seg], ax
    pop ax
    jmp .next_exec
.exec_flip:
    mov ah, 0x31
...
.exec_frect:
    ...
    call eval_value
    push ax             ; 压入 H
    call eval_value     ; 计算 Color (结果留在 AL 中，不需要 push)
    pop di              ; 弹出 H
    pop si              ; 弹出 W
    pop dx              ; 弹出 Y
    pop cx              ; 弹出 X
...
.pi_loop2:
    ...
    popa
    ret
msg_banner    db "PASIC 26JUL1.5 ", 13, 10, "Ready.", 13, 10, 0
msg_prompt    db "> ", 0
